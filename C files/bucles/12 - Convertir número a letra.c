/* Programa escribir_nmeros
   Lee un nmero entero de hasta 5 cifras y lo escribe en forma de texto
   (Por ejemplo, el n 4950 producirá la salida: "cuatro mil novecientos cincuenta")
*/
...
int main(void)
{
	...
	do
	{
	  ...
	  scanf("%d",&numero);
	  extraer_digitos(numero, &unid, &dec, &cent, &umill, &dmill);  /* Separar el n en dígitos */
...
/* Extraer los dígitos del nmero mediante divisiones sucesivas */
void extraer_digitos(int numero, int* U, int* D, int* C, int* UM, int* DM)
{
...
/* Escribe las centenas. La nica excepción es 100 */
void escribir_centenas(int unid, int dec, int cent)
{
...
/* printf las decenas. Los nmeros del 10 al 19 se tratan como excepciones. También el 20 frente al caso general (2x = "veinti"). En el resto de decenas, se escribe una " y " si la unidad no es cero (ej: 3x = "treinta y ") */
void escribir_decenas(int unid, int dec)
{
	if (dec == 1)
		switch (unid)
