...
void seleccion(void)
{
	...
	printf("ingrese la cantidad de numeros para ordenar de forma ascendente:\n");
	scanf("%d",&cant);
	system("cls");
	for(i=0;i<cant;i++)
	{
	...
	{
		min = i;
		//ciclo para realizar la condicional
		for(j=i+1;j<=cant;j++)
		{
			//si numeros en la posicion j es menor a el valor almacenado en min 
			if(numeros[j] < numeros[min])
			{
				//si lo es la variable min pasara a valer lo que habia en la posicion j
				min = j;
			}
		}
		aux = numeros[i];
