...
int main(void)
{
    ...
    printf("ejemplos de funciones recursivas para Srboles\n\n");
    printf("Generando un Arbol aleatorio de %i elementos...\n", TAM);
    t1 = time(NULL);
    srand(t1);
    ...
    printf("\n\nProfundidad del Arbol: %i\n", profundidad(raiz, 0));
    
    if (completo(raiz))
        printf("\n\nEl Arbol esta� COMPLETO\n\n");
    else
        printf("\n\nEl Arbol esta� INCOMPLETO\n\n");
...
int completo(struct s_nodo *nodo)
{
    ...
    if (nodo != NULL)
    {
        if ((nodo->hijo_izq != NULL) && (nodo->hijo_der != NULL))
                ...
                // Nodo con CERO hijos. Esta� completo por definicion
                c = 1;
        else    // Nodo con UN hijo. No esta� completo por definicion
                c = 0;
    }
    else        // Si nodo == NULL, este subArbol si� esta completo (porque esta� vacio)
        c = 1;
